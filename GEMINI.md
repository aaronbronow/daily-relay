...
- **Ollama Prompt Benchmarking & Optimization**: Developing an automated evaluation tool (`tests/promptOptimizer.js`) using a combination of deterministic regular expression constraints and LLM-as-a-judge scoring is a highly robust method to optimize system prompts. Testing structured bullet point formats against paragraph summaries showed a **56% decrease in latency** (1.87s vs 4.28s) and higher information density without build number leakage.
- **Google Docs HTML Sync & Conversion**: Uploading rendered HTML to Google Drive with `mimeType: 'application/vnd.google-apps.document'` in metadata and `mimeType: 'text/html'` in media automatically triggers native document conversion on Google's end. Utilizing `drive.files.update` with streamed text allows seamless overwriting and content sync of the existing Google Doc without duplicate file accumulation.
- **Docker Volume Sync Detection**: Mount core configuration files (like `package.json`, `Dockerfile`, `entrypoint.sh`) as read-only volumes (e.g. to `/app/*.host`) to compare them with built-in versions on startup or aggregation. This determines if the host configuration files have diverged from the built image environment, allowing the system to alert the user to rebuild the container without external tools or complex git-parsing.
- **Target-Specific Daily Publishing Limits**: Persisting publishing timestamps in the shared local `cache.json` using the local ISO standard representation (`toLocaleDateString('en-CA')`) enables seamless, stateful once-daily publishing limits on specific targets (like Google Docs) without breaking the frequent execution/generation cadence of the main UI dashboard. By returning delivery receipts/booleans from downstream publisher modules back to the core aggregator orchestrator, state transitions are only updated on guaranteed delivery success.
- **SSE Client ID Collision Prevention**: Using `Date.now()` for Server-Sent Events (SSE) client IDs is vulnerable to collisions when multiple clients connect at the exact same millisecond. If a collision occurs, a client disconnect triggers the removal of all clients sharing that ID from the server's client array, which leaves orphaned active connections. Using a simple sequential counter (`nextClientId++`) guarantees unique IDs and prevents resource leaks.
- **Item-Level Summary Caching**: Comparing incoming collector items against the cached item summaries (by unique `url` or `title` + `timestamp` fallback) allows the aggregator to bypass Ollama LLM requests for already-summarized items when a feed updates. This achieves an ~80% reduction in API calls and latency during frequent aggregations.
